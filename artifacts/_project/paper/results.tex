...
\paragraph{Robustness and outliers.}
Trimmed and Winsorized estimates confirm that the Qwen effects are stable (Appendix~\ref{app:robustness}). The Llama SVG $\Delta$ is sensitive to extreme values ($-$0.8\,pp raw vs.\ $+$7.6\,pp trimmed), and Qwen SVG precise is dragged negative by a single outlier (sample~\#54, truncated at 8192 tokens, per-sample reduction $= -10.0$).
