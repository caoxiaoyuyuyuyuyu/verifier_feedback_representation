...
For Qwen, this domain$\times$specificity interaction reaches statistical significance. The sign test confirms that generic feedback significantly outperforms precise on SVG (46 vs.\ 9 wins, $p < 0.001$), while precise significantly outperforms generic on Python (9 vs.\ 1 wins, $p = 0.021$; Wilcoxon $p = 0.041$). For Llama, the directional pattern is consistent with Qwen but does not reach significance, likely due to Llama's stronger baseline self-correction ability on SVG (40.6\% vs.\ Qwen's $-$3.2\%), which compresses the room for differential effects.
...
In the SVG geometric domain, diagnostics are \emph{structural}: a predicate failure (``circles are not externally tangent'') implicates global relationships between multiple elements, and the fix often requires coordinated adjustments to several attributes (centres, radii, viewBox). Precise feedback still localises the failure to specific element IDs and reports metric values, but the model must reason about multi-element geometry to translate this information into a valid edit. For Qwen---whose SVG correction ability is near zero (DRR $= -3.2$\%)---the additional precise details may overwhelm the model's limited geometric reasoning capacity, producing responses that are longer (token count at the 8192 limit) but no more correct.

\subsection{Why Hybrid Amplifies Specificity for Qwen but Not Llama}

The hybrid format provides diagnostic information twice: once as a natural-language narrative and once as a structured JSON block. For Qwen, this redundancy appears to reinforce the corrective signal, enabling the model to cross-reference the human-readable description with the machine-parseable fields. The result is the strongest effect we observe: hybrid precise achieves 75.5\% DRR, 8.2\,pp above NL precise and 16.0\,pp above raw JSON precise.

For Llama, the same redundancy does not help. Llama's stronger baseline self-correction (NL generic already at 70.4\% vs.\ Qwen's 55.1\%) suggests that the model requires less external guidance. When presented with hybrid precise feedback---the most information-dense representation---Llama's DRR \emph{drops} to 69.0\%, below every other Llama configuration. We hypothesise that Llama's instruction-following behaviour is disrupted by excessively detailed feedback: rather than selectively attending to the most actionable fields, the model may attempt to address every piece of information, leading to suboptimal edits.

This interpretation is consistent with recent observations that larger or better-trained models can exhibit \emph{prompt sensitivity} where additional context degrades rather than improves performance \cite{shi2023large}.
...
\begin{enumerate}
    \item \textbf{Model capability.} Weaker models (Qwen) benefit from maximum feedback specificity and the hybrid format, while stronger models (Llama) perform best with generic feedback in a structured format.
