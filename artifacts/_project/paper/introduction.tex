...
\item \textbf{Domain-dependent specificity.} Evidence that the effect of feedback detail reverses between domains: precise diagnostics help rule-based tasks (Python security, $+12.2$\,pp for Qwen) but hurt structural tasks (SVG geometry, $-19.9$\,pp), with opposing directions of significance suggesting a domain$\times$specificity interaction for Qwen (\S\ref{sec:results-phaseA}).
